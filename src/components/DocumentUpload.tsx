...
import { uploadFile } from '@/store/useDataStore';
...
const DocumentUpload = ({ 
  ...
  const processFile = async (uploadedFile: File) => {
    ...
    try {
        setProgress(50);
        const url = await uploadFile(uploadedFile, `${storagePath}/${Date.now()}_${uploadedFile.name}`);
        setProgress(100);
        setUploadedUrl(url);
        ...
        {!file ? (
            <div 
                className={cn(
                    ...
                    isUploading && "ai-pulse border-blue-400 pointer-events-none"
                )}
            ...
            >
                ...
                <p className="relative z-10 text-xs font-bold text-slate-400 mt-2 uppercase tracking-widest">PNG o JPG hasta 10MB</p>
                ...
                <div className="flex items-center justify-between p-6 bg-slate-50 rounded-[1.5rem] border border-slate-100 shadow-sm">
                    <div className="flex items-center gap-5">
                         ...
                         <div>
                             ...
                             <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">{(file.size / 1024 / 1024).toFixed(2)} MB</p>
                         </div>
                    </div>
                ...
                {isUploading && (
                    <div className="bg-blue-50/50 p-6 rounded-[1.5rem] border border-blue-100/50 space-y-4 shadow-inner">
                        <div className="flex justify-between items-center text-xs font-black uppercase tracking-[0.2em] text-blue-600">
                            <span className="flex items-center gap-2">Subiendo archivo...</span>
                            <span>{progress}%</span>
                        </div>
